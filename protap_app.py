...
def create_pdf_report(project_info, params, results):
    """Generates a PDF report using fpdf2 and returns it as bytes."""
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.cell(0, 8, "ASME B31.8 - Branch Connection Reinforcement Calculation", new_x="LMARGIN", new_y="NEXT", align="C")
    pdf.ln(5)
    
    # --- Project Information ---
    pdf.set_font("Helvetica", style="B", size=12)
    pdf.cell(0, 8, "Project Information", new_x="LMARGIN", new_y="NEXT", border="B")
    pdf.set_font("Helvetica", size=10)
    
    for key, value in project_info.items():
        pdf.cell(50, 6, str(key), align="L")
        pdf.cell(0, 6, f": {value}", new_x="LMARGIN", new_y="NEXT", align="L")
    pdf.ln(5)
    
    # --- Calculation Results ---
    pdf.set_font("Helvetica", style="B", size=12)
    pdf.cell(0, 8, "Calculation Results", new_x="LMARGIN", new_y="NEXT", border="B")
    pdf.set_font("Helvetica", size=10)
    
    # Status Banner
    pdf.set_font("Helvetica", style="B", size=11)
    pdf.cell(50, 8, "Overall Result", align="L")
    pdf.cell(0, 8, f": {results['result']}", new_x="LMARGIN", new_y="NEXT", align="L")
    pdf.set_font("Helvetica", size=10)

    # Key Metrics
    metrics = [
        ("Header OD", f"{results['header_od_mm']:.1f} mm"),
        ("Branch OD (Db)", f"{results['Db_mm']:.1f} mm"),
        ("Required WT (t)", f"{results['t_required_mm']:.2f} mm"),
        ("Area to be Reinforced (Ar)", f"{results['Ar_mm2']:.2f} mm2"),
        ("Header Area (A1)", f"{results['A1_mm2']:.2f} mm2"),
        ("Nozzle Area (A2')", f"{results['A2_prime_mm2']:.2f} mm2"),
        ("Reinforcement Area (A3')", f"{results['A3_prime_mm2']:.2f} mm2"),
        ("Ar - (A1+A2'+A3')", f"{results['control_mm2']:.2f} mm2"),
        ("Total Weight Estimate", f"{results['total_weight_kg']:.2f} kg")
    ]
    ...
    pdf.multi_cell(0, 5, "We hereby certify that these calculations are based on the specification referenced above and conform to the standards referenced therein.")
